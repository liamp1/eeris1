...
class ReceiptConsumer(AsyncWebsocketConsumer):
    ...
    async def poll_for_receipt_data(self, receipt_id):
            ...
            try:
                ...
                for i in range(100):  # 100 attempts * 3 seconds = 5 minutes
                    print(f"Checking receipt data for {receipt_id} (attempt {i+1}/100)")
                    
                    try:
                        # Wrap the check in a try-except to catch any errors
                        ...
                        await self.send(text_data=json.dumps({
                            'type': 'error',
                            'message': f"Error checking receipt: {str(e)}",
                            'receipt_id': receipt_id
                        }))
                        return
                ...
                print(f"Unexpected error in polling task: {e}")
